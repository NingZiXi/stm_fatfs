...
当前发布版本为 **v1.0.0**，默认使用 FatFs R0.14b。组件目录中的 `fatfs` 源码是可审计的固定副本，保留 ChaN 的授权头；原创粘合代码采用本目录的 MIT 许可。应用也可以通过 `STM_FATFS_SOURCE_DIR` 提供自己的 FatFs 源码或复用已经存在的 `fatfs` target。
